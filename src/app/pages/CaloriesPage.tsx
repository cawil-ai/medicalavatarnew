...
const loadCalorieData = async () => {
  try {
    ...
    console.log('ALL calorie logs:', resAll.documents);

    // ✅ FILTER TODAY ONLY
    const todayMeals = resAll.documents.filter(
      d => d.date === today
    );
    ...
    const mapped: Meal[] = todayMeals.map(d => ({
      ...
      calories: d.calories || 0,
      time: d.mealTime || '',
      date: d.date // ✅ ADD THIS
    }));

    setMeals(mapped);

    // ✅ STREAK USES ALL DATA
    setStreak(calculateWeeklyStreak(resAll.documents));
    console.log("Unique days:", [...new Set(resAll.documents.map(d => d.date))]);
    const weeklyCalories = getWeeklyCalories(resAll.documents);
